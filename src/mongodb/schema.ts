...
export interface MongoDBFieldSchema {
    field: string;
    type: string;
    isRequired: boolean;
    subFields?: MongoDBFieldSchema[];
}

export interface MongoDBCollectionSchema {
    collection: string;
    fields: MongoDBFieldSchema[];
    count: number;
    indexes?: unknown[];
}
...
export function inferSchemaFromDocument(
    ...
    const schema: MongoDBFieldSchema[] = [];
  
    for ( const [ key, value ] of Object.entries( doc ) ) {
        const fieldPath = parentPath ? `${parentPath}.${key}` : key;
        const fieldType = inferSchemaFromValue( value );
        const field: MongoDBFieldSchema = {
            field: fieldPath,
            type: fieldType,
            isRequired: true,
        };
...
export async function buildCollectionSchema(
    ...
    const fields: MongoDBFieldSchema[] = Array.from(fieldSchemas.entries()).map(
        ( [ field, types ] ) => ( {
            field,
            type:
                types.size === 1
